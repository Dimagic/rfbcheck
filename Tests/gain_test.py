...
class GainTest(QtCore.QThread):
    def __init__(self, testController, mainParent, parent = None):
        ...
        if self.mainParent.stopTestFlag == True:
            return
    ...
    def gainTest(self,freq,gainMin,gainMax):
        ...
        if gainMin <= currentGain <= gainMax:
            ...
            if  q == QMessageBox.Retry:
                ...
                self.testController.resSignal.emit('Gain',self.whatConn,str(gainMin),str(currentGain),str(gainMax),0)
            elif q == QMessageBox.Cancel:
                self.testController.resSignal.emit('Gain',parent.whatConn,str(gainMin),str(currentGain),str(gainMax),0)
