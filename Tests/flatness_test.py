...
class FlatnessTest(QtCore.QThread):
    def __init__(self, testController, mainParent, parent = None):
        ...
        if testController.stopTestFlag == True:
        ...
        self.ser = self.mainParent.ser.ser

        if self.mainParent.whatConn == "Dl":
            self.mainParent.testLogDl.update({'SN':self.mainParent.rfbSN.text()})
            self.flatnessTest(self.mainParent,self.mainParent.instr.sa,self.mainParent.instr.gen,self.mainParent.listSettings[1],self.mainParent.atrSettings.get('flat_dl_max'))
        elif self.mainParent.whatConn == "Ul":
            self.mainParent.testLogUl.update({'SN':self.mainParent.rfbSN.text()})
            self.flatnessTest(self.mainParent,self.mainParent.instr.sa,self.mainParent.instr.gen,self.mainParent.listSettings[2],self.mainParent.atrSettings.get('flat_ul_max'))
        else:
            self.testController.msgSignal.emit("w","Warning","gainFlatness_test Dl/Ul",1)
    ...
    def flatnessTest(self, parent, sa, gen, freq, flat):
        ...
        self.testController.logSignal.emit("MIN = "+str(genPow - minGain)+" dBm on freq = " +str(minFreq)+" MHz",0)
        self.testController.logSignal.emit("MAX = "+str(genPow - maxGain)+" dBm on freq = " +str(maxFreq)+" MHz",0)
    ...
    def getFlatness(self,parent,sa,gen,start,stop,freq):
        ...
        self.testController.logSignal.emit("RG-test start Freq: " + str(start) + " MHz; end Freq: " + str(stop) + " MHz",0)
