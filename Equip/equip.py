...
def setAmplTo(self, conn, cmd, instr, sa, gen, ampl,parent):
    ...
    try:
        ...
        while abs(abs(gain) - abs(ampl)) > 0.03:
            ...
            #print(delta)
            if   delta <= 0.5: steep = 0.01
            elif delta <= 2:   steep = 0.5
            else:              steep = 1
            if gain < ampl:
                genPow += steep
            else:
            ...
            parent.myThread.progressBarSignal.emit('Set ampl. to ' + str(ampl) + ' dB',0,0)# if err/////

    except Exception as e:
            parent.sendMsg('c','Set amlitude error',str(e),1)
            return
    parent.sendLog('Current ampl: ' + str(round(gain,2)) + ' dBm',0)
...
##    print(freqArrStr)
    for n in freqArrStr:
        ...
        rows = cursor.execute("select * from "+table+" where freq >= :s and freq <= :e",{'s':start,'e':stop}).fetchall()
...
def setDSA(conn, cmd, whatConn, dsa1, dsa2, dsa3):
    ...
    toSend = toSend +str(floatToHex(dsa1).replace('0x', ''))+str(floatToHex(dsa2).replace('0x', ''))+str(floatToHex(dsa3).replace('0x', ''))
    if sum%256 < d:
