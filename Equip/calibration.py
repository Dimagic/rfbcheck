...
##                print(bandList)
                ...
                if parent.calAllBands.isChecked() == True:
                    tmp = cursor.execute("select freq_band_dl_1, freq_band_dl_2, freq_band_ul_1, freq_band_ul_2 from atr")
                else:
                    tmp = cursor.execute("select freq_band_dl_1, freq_band_dl_2, freq_band_ul_1, freq_band_ul_2 from atr where rfb_type = :s",{'s':parent.rfbTypeCombo.currentText()})
                for i in tmp:
                    for j in i:
        ...
        if parent.sendMsg('i','Message','Connect SA to Gen using cabble whith attenuator',2) == QMessageBox.Cancel: return
        table = 'calSaToGen'
        for i in bandList:
            self.makeCalibr(parent,table,int(parent.saAtten.text()),i)


        if parent.sendMsg('i','Message','Connect Gen to SA using cabble whithout attenuator',2) == QMessageBox.Cancel: return
        table = 'calGenToSa'
        for i in bandList:
    ...
    def makeCalibr(self,parent,table,attenuator,freqArr):
        ...
        self.t = Thread(target=self.runCalibration, args=(parent,currBand,startFreqFull,stopFreqFull,centerFreq,attenuator,arrFreq,query,))
        self.t.start()
...
##                time.sleep(0.2)
    ...
    def strToFreq(self,parent,curRange):
        try:
            ind = curRange.find('@')
