...
class Journal:
    ...
    def feelJournal():
        ...
        q1 = "select * from test_results where cast(substr(dateTest,0,9) AS INTEGER) between %s and %s" \
             % (dateBegin, dateEnd)
        q = "select * from (" + q1 + ") where (rfb_type like '%" + Journal.parent.journalFilter.text() + \
            "%' or sn like '%" + Journal.parent.journalFilter.text() + "%') order by dateTest DESC"
    ...
    def deleteRecords(self):
        ...
        q = self.parent.sendMsg('i', 'Delete record',
                                'Will be remove test record for RF board %s. Are you sure?' % (sn), 2)
        if q == QtWidgets.QMessageBox.Ok:
            try:
                ...
                self.parent.sendMsg('c', 'Delete records error', str(e), 1)
